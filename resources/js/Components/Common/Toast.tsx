...
interface CustomToastPassThroughMethodOptions
    extends ToastPassThroughMethodOptions {
    index: number;
}
        ...
        <div className="w-full flex flex-row items-center gap-1.5">
            ...
            <div className="flex w-full flex-col gap-0.5">
                <h1 className="text-sm font-bold">
                    {ucFirst(flash_message.type)}
                </h1>
                <p className="text-xs font-semibold line-clamp-2">{flash_message.message}</p>
            </div>
        </div>
    );
    ...
    const passT: ToastPassThroughOptions = {
        message: ({ state, index }: CustomToastPassThroughMethodOptions) => ({
            className: classNames(
                "mb-4 opacity-80 backdrop-blur rounded-md w-full",
                {
        ...
        content: () => ({
            className: classNames(
                "flex justify-between items-center py-3 pl-4 pr-3.5"
            ),
        }),
        closeButton: ({
            state,
            index,
        }: CustomToastPassThroughMethodOptions) => ({
